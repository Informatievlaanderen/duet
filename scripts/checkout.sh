...
mkdir -p $ROOTDIR
curl -o $ROOTDIR/commit.json https://raw.githubusercontent.com/Informatievlaanderen/duet-generated/$CIRCLE_BRANCH/report/commit.json
sleep 5s
...
then
	...
	pushd $ROOTDIR/$MAIN/$RDIR
	   ...
	   echo ${row} | base64 --decode | jq --arg comhash "${comhash}" --arg toolchainhash "${toolchainhash}" '. + {documentcommit : $comhash, toolchaincommit: $toolchainhash, hostname: "https://duet.dev-vlaanderen.be" }' > .publication-point.json
	   cleanup_directory 
        popd
